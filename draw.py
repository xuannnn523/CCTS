...
def plot_scores_for_all_datasets(method, names, titles, data_base_dir, result_base_dir):
    ...
    for i, (dataset, title) in enumerate(zip(names, titles)):
        ...
        baseline_scores = parse_community_results(baseline_file)

        # 准备箱线图数据
        results = {
            "Objective Score": [baseline_scores, naive_scores, fast_scores]  # 顺序调整为 baseline, naive, fast
        }
        ...
        axs[row, col].set_xticks([1, 2, 3])  # 确保x刻度正常显示
