...
def plot_thresholds_for_all_datasets(method, names, titles, data_base_dir):
    ...
    for i, (dataset, title) in enumerate(zip(names, titles)):
        ...
        # 准备箱线图数据
        results = [naive_thresholds, fast_thresholds]
        ...
        # 修复y轴显示问题
        if naive_thresholds and fast_thresholds:
            y_min = min(min(naive_thresholds), min(fast_thresholds))
            y_max = max(max(naive_thresholds), max(fast_thresholds))
        ...
        axs[row, col].set_xticklabels(labels, fontsize=10)  # 显示 Naive 和 Fast
