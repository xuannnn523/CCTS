...
import numpy as np
import alphashape  # Alpha形状包
import random
import time
import math
from collections import deque
method=['Louvain','Girvan','LPA','FN']
num = 2
...
if __name__ == '__main__':
    ...
    method=f'{method[num]}'
    with open(f'./community_partitions/{method}/{dataset_name}_partition.pkl', 'rb') as f:
    ...
    if if_write:
        ...
        with open(f'result/{method}/{dataset_name}_{param}.txt', 'w', encoding='utf-8') as file:
            # 输出每个社区的结果
            for result in community_results:
    ...
    # # 收集各项指标
    # scores = [result['得分'] for result in community_results]
    # coverages = [result['覆盖率'] for result in community_results]
    # error_rates = [result['错误率'] for result in community_results]
    # thresholds = [result['阈值'] for result in community_results]
    #
    # # 创建箱线图
    # fig, axs = plt.subplots(1, 3, figsize=(15, 5))
    #
    # # 得分分布
    # axs[0].boxplot(scores, vert=True, patch_artist=True, showmeans=True)
    # # axs[0].set_title("Score Distribution")
    # axs[0].set_ylabel("Object Score")
    #
    # # 覆盖率分布
    # axs[1].boxplot(coverages, vert=True, patch_artist=True, showmeans=True)
    # # axs[1].set_title("Coverage Rate Distribution")
    # axs[1].set_ylabel("Precision")
    #
    # # 错误率分布
    # axs[2].boxplot(error_rates, vert=True, patch_artist=True, showmeans=True)
    # # axs[2].set_title("Error Rate Distribution")
    # axs[2].set_ylabel("Error Rate")
    # plt.tight_layout()
    #
    # # 构造保存路径
    # result_dir = f'result/{method}/boxplot'  # 存储结果的主目录
    # if not os.path.exists(result_dir):
    #     os.makedirs(result_dir)  # 如果路径不存在，则创建
    # if(if_part == False):
    #     pdf_path = f'{result_dir}/{dataset_name}_boxplot.pdf'
    # else:
    #     pdf_path = f'{result_dir}/{dataset_name}_boxplot_fast.pdf'
    #
    #
    # # 保存为 PDF 文件
    # plt.savefig(pdf_path, format='pdf', bbox_inches='tight')
    # print(f"Boxplot saved at {pdf_path}")
    #
    #
    # plt.show()
    # #